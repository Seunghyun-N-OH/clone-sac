...
public class EventE {
    ...
    @JoinColumn(name = "poster_id") // 1:1 [포스터는 1개만 등록받을 것]
    private EventPoster poster; // TODO 파일 받을 때 이미지파일 맞는지 파일의 타입체크 기능 만들어
    // 포스터

    @OneToMany // 1:N [상세페이지는 여러개가 될 수 있음]
    @JoinColumn(name = "event_id")
    @Builder.Default
    private List<EventDetailImg> detail_img = new ArrayList<>(); // TODO 파일 받을 때 이미지파일 맞는지 파일의 타입체크 기능 만들어
    // 상세페이지
